...
export function useMarkets() {
  ...
  useEffect(() => {
    console.log('Markets state changed:', markets.length, 'markets')
  }, [markets])
  ...
  const fetchMarkets = async () => {
    try {
      ...
      console.log('API Data received:', data.length, 'markets')
      console.log('First market:', data[0])
      setMarkets(data)
      setError(null)
    } catch (err) {
  ...
  const placeBet = async (marketId: string, outcome: string, amount: number) => {
    try {
      ...
      // Refresh markets to get updated data
      await fetchMarkets()
